...
const entityTypes = computed(() => {
  ...
  const fallbackColors = ['#FF6B35', '#004E89', '#7B2D8E', '#1A936F', '#C5283D', '#E9724C']
  let fallbackIdx = 0

  props.graphData.nodes.forEach(node => {
    const type = node.labels?.find(l => l !== 'Entity') || 'Entity'
    if (!typeMap[type]) {
      const color = semanticColors[type] || fallbackColors[fallbackIdx++ % fallbackColors.length]
...
const renderGraph = () => {
  ...
  const getNodeRadius = (id) => {
    ...
    return 7 + Math.round((deg / maxDegree) * 10)
  }
