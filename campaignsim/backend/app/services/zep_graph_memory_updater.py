...
Writes agent social actions as episodes into the Zep knowledge graph.
"""
...
logger = get_logger('campaignsim.zep_graph_memory_updater')

@dataclass
class AgentActivity:
    """Represents one action taken by a simulation agent."""
    ...
    def to_episode_text(self) -> str:
        """Convert this action to a natural-language episode string for Zep."""
        action_descriptions = {
    ...
    def _describe_quote_post(self) -> str:
        ...
        if original_content and original_author:
            ...
            base = f'quoted: "{original_content}"'
        elif original_author:
...
class ZepGraphMemoryUpdater:
    """
    Writes agent simulation actions as episodes into the Zep knowledge graph.

    Buffers actions by platform (BATCH_SIZE at a time) and sends them to Zep
    in batches. Each episode is a natural-language description built from
    action_args including post content, author names, and comment content.
    """
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                if attempt < self.MAX_RETRIES - 1:
                    logger.warning(f"Zep batch send failed (attempt {attempt + 1}/{self.MAX_RETRIES}): {e}")
                    time.sleep(self.RETRY_DELAY * (attempt + 1))
                else:
                    logger.error(f"Zep batch send failed after{self.MAX_RETRIES} retries: {e}")
    ...
    def _flush_remaining(self):
        ...
        with self._buffer_lock:
            for platform, buffer in self._platform_buffers.items():
                if buffer:
                    display_name = self._get_platform_display_name(platform)
                    logger.info(f"Sending{display_name}platform remaining {len(buffer)} ")
