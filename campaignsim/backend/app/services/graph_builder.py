...
class GraphBuilderService:
    ...
    def build_graph_async(
        ...
        graph_name: str = "CampaignSim Graph",
        chunk_size: int = 500,
    ...
    def create_graph(self, name: str) -> str:
        """Zep"""
        graph_id = f"campaignsim_{uuid.uuid4().hex[:16]}"
        
        self.client.graph.create(
            graph_id=graph_id,
            name=name,
            description="CampaignSim Social Simulation Graph"
        )
    ...
    def set_ontology(self, graph_id: str, ontology: Dict[str, Any]):
        """..."""
        # Pass the raw ontology dict directly to the KGClient adapter,
        # which normalises it and stores as JSON (no Pydantic class construction needed).
        if ontology:
