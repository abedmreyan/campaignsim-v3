...
class ParallelIPCHandler:
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        if success_count > 0:
            ...
            print(f" Interview complete: agent_id={agent_id}, ={success_count}/{len(platforms_to_interview)}")
            return True
        else:
            errors = [f"{p}: {r.get('error', '')}" for p, r in results["platforms"].items()]
            self.send_response(command_id, "failed", error="; ".join(errors))
            print(f" Interview failed: agent_id={agent_id}, ")
            return False
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        if twitter_interviews and self.twitter_env:
            try:
                ...
                print(f" TwitterInterview failed: {e}")
        ...
        if reddit_interviews and self.reddit_env:
            try:
                ...
                print(f" RedditInterview failed: {e}")
        ...
        if results:
            ...
            print(f" Interview complete: {len(results)} agents")
            return True
        else:
            self.send_response(command_id, "failed", error="")
            return False
...
async def run_twitter_simulation(
    ...
    if not os.path.exists(profile_path):
        log_info(f": ProfileFile not found: {profile_path}")
        return result
    ...
    for round_num in range(total_rounds):
        if _shutdown_event and _shutdown_event.is_set():
            if main_logger:
                main_logger.info(f" after attempt {round_num + 1} ")
            break
...
async def main():
    ...
    log_manager.info(f"File: {args.config}")
