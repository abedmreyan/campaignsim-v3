  ...
  <div class="env-setup-panel">
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/ab_test</p>
          <p class="description">Define your campaign variants and launch a parallel A/B simulation. Each variant runs independently — results are compared and ranked.</p>

          <!-- Campaign meta -->
          <div v-if="phase >= 4 && !abLaunched" class="ab-launcher">
            <div class="ab-meta-row">
              <div class="ab-field">
                <label class="ab-label">Brand Name</label>
                <input class="ab-input" v-model="campaignMeta.brand_name" placeholder="e.g. FreshBrew Coffee" />
              </div>
              <div class="ab-field ab-field--wide">
                <label class="ab-label">Campaign Goal</label>
                <input class="ab-input" v-model="campaignMeta.campaign_goal" placeholder="e.g. Drive trial purchases among health-conscious millennials" />
              </div>
            </div>
              ...
              <Transition name="fade" mode="out-in">
                <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                  ...
                  <div class="range-wrapper">
                    <input type="range" v-model.number="customMaxRounds" min="10" :max="autoGeneratedRounds" step="5" class="minimal-slider" :style="{ '--percent': ((customMaxRounds - 10) / (autoGeneratedRounds - 10)) * 100 + '%' }" />
                    <div class="range-marks">
                      <span>10</span>
                      <span class="mark-recommend" :class="{ active: customMaxRounds === 40 }" @click="customMaxRounds = 40" :style="{ position: 'absolute', left: `calc(${(40 - 10) / (autoGeneratedRounds - 10) * 100}% - 30px)` }">{{ $t('step2.recommendedRounds', { rounds: 40 }) }}</span>
            ...
            <div class="ab-variants-section">
              <div class="ab-variants-header">
                <span class="ab-variants-title">Campaign Variants</span>
                <button v-if="abVariants.length < 3" class="ab-add-btn" @click="addVariant">+ Add Variant</button>
              </div>

              <div class="ab-variant-card" v-for="(v, idx) in abVariants" :key="idx">
                <div class="ab-variant-header">
                  <input class="ab-variant-name-input" v-model="v.variant_name" placeholder="Variant name" />
                  <button v-if="abVariants.length > 2" class="ab-remove-btn" @click="removeVariant(idx)">×</button>
                </div>

                <div class="ab-variant-fields">
                  <div class="ab-field">
                    <label class="ab-label">Channel</label>
                    <select class="ab-select" v-model="v.channel">
                      <option v-for="ch in CHANNELS" :key="ch" :value="ch">{{ ch.charAt(0).toUpperCase() + ch.slice(1) }}</option>
                    </select>
                  </div>
                  <div class="ab-field">
                    <label class="ab-label">Format</label>
                    <select class="ab-select" v-model="v.content.format">
                      <option v-for="fmt in CONTENT_FORMATS" :key="fmt" :value="fmt">{{ fmt }}</option>
                    </select>
                  </div>
                  <div class="ab-field">
                    <label class="ab-label">Tone</label>
                    <select class="ab-select" v-model="v.content.tone">
                      <option v-for="tone in TONES" :key="tone" :value="tone">{{ tone.charAt(0).toUpperCase() + tone.slice(1) }}</option>
                    </select>
                  </div>
                  <div class="ab-field ab-field--wide">
                    <label class="ab-label">Target Segment</label>
                    <input class="ab-input" v-model="v.target_segment" placeholder="e.g. Health-conscious millennials 25–35" />
                  </div>
                  <div class="ab-field ab-field--wide">
                    <label class="ab-label">Headline</label>
                    <input class="ab-input" v-model="v.content.headline" placeholder="e.g. Zero Sugar. Zero Wait." />
                  </div>
                  <div class="ab-field ab-field--full">
                    <label class="ab-label">Ad Body</label>
                    <textarea class="ab-textarea" v-model="v.content.body" rows="2" placeholder="Campaign message body..."></textarea>
                  </div>
                  <div class="ab-field">
                    <label class="ab-label">CTA</label>
                    <input class="ab-input" v-model="v.content.cta" placeholder="e.g. Try it — 20% off" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="action-group dual">
              <button class="action-btn secondary" @click="$emit('go-back')">← {{ $t('step2.backToGraphBuild') }}</button>
              <button class="action-btn primary" :disabled="abLaunching || !campaignMeta.brand_name" @click="launchAbTest">
                <span v-if="abLaunching">Launching...</span>
                <span v-else>Launch A/B Test ➝</span>
              </button>
            </div>
          </div>

          <!-- Waiting for phase 4 -->
          <div v-else-if="phase < 4" class="ab-waiting">
            <p class="description">Waiting for persona generation to complete before launching campaign...</p>
            <div class="action-group dual">
              <button class="action-btn secondary" @click="$emit('go-back')">← {{ $t('step2.backToGraphBuild') }}</button>
            </div>
          </div>
          ...
          <!-- Running / Done state -->
          <div v-else class="ab-running">
            <div class="ab-status-header">
              <span class="ab-status-label">Campaign ID</span>
              <code class="ab-campaign-id">{{ abCampaignId }}</code>
            </div>

            <div class="ab-variant-status-list">
              <div class="ab-variant-status" v-for="v in abVariantStatuses" :key="v.variant_id">
                <div class="avs-info">
                  <span class="avs-name">{{ v.variant_name || v.variant_id }}</span>
                  <span class="avs-channel">{{ v.channel }}</span>
                </div>
                <div class="avs-status" :class="v.status">
                  <span v-if="v.status === 'running'" class="avs-dot running"></span>
                  <span v-else-if="v.status === 'completed'" class="avs-dot done"></span>
                  <span v-else class="avs-dot failed"></span>
                  {{ v.status }}
                </div>
              </div>
            </div>

            <div v-if="!abAllDone" class="ab-poll-note">Polling for results every 5 seconds...</div>

            <div class="action-group dual" style="margin-top: 1.5rem;">
              <button class="action-btn secondary" @click="$emit('go-back')">← Back</button>
              <button class="action-btn primary" :disabled="!abAllDone" @click="goToReport">
                {{ abAllDone ? 'View Recommendations →' : 'Waiting for variants...' }}
              </button>
            </div>
          </div>
        </div>
...
// A/B Campaign Launcher state
const campaignMeta = ref({ brand_name: '', campaign_goal: '' })
const abVariants = ref([
  { variant_name: 'Variant A', channel: 'instagram', target_segment: '', content: { format: 'VideoAd', headline: '', body: '', cta: '', tone: 'playful' } },
  { variant_name: 'Variant B', channel: 'tiktok', target_segment: '', content: { format: 'CarouselPost', headline: '', body: '', cta: '', tone: 'inspirational' } }
])
const abLaunching = ref(false)
const abLaunched = ref(false)
const abCampaignId = ref(null)
const abVariantStatuses = ref([])
const abAllDone = ref(false)
let abPollTimer = null

const CHANNELS = ['instagram', 'tiktok', 'twitter', 'linkedin', 'email']
const CONTENT_FORMATS = ['VideoAd', 'CarouselPost', 'StaticImage', 'StoryAd', 'EmailBlast']
const TONES = ['playful', 'professional', 'inspirational', 'urgent', 'friendly', 'neutral']
...
// A/B Campaign methods
const addVariant = () => {
  if (abVariants.value.length >= 3) return
  abVariants.value.push({
    variant_name: `Variant ${String.fromCharCode(65 + abVariants.value.length)}`,
    channel: 'linkedin',
    target_segment: '',
    content: { format: 'StaticImage', headline: '', body: '', cta: '', tone: 'professional' }
  })
}

const removeVariant = (idx) => {
  if (abVariants.value.length <= 2) return
  abVariants.value.splice(idx, 1)
}

const launchAbTest = async () => {
  if (!props.simulationId) return
  abLaunching.value = true
  addLog('Launching A/B campaign test...')
  try {
    const payload = {
      simulation_id: props.simulationId,
      brand_name: campaignMeta.value.brand_name,
      campaign_goal: campaignMeta.value.campaign_goal,
      variants: abVariants.value.map(v => ({
        variant_name: v.variant_name,
        channel: v.channel,
        target_segment: v.target_segment,
        max_rounds: useCustomRounds.value ? customMaxRounds.value : (autoGeneratedRounds.value || 20),
        content: { ...v.content }
      }))
    }
    const res = await runAbTest(payload)
    if (res.success && res.data) {
      abCampaignId.value = res.data.campaign_id
      abVariantStatuses.value = res.data.variants || []
      abLaunched.value = true
      addLog(`Campaign launched: ${abCampaignId.value}`)
      addLog(`Running ${abVariantStatuses.value.length} variant(s) in parallel...`)
      startAbPolling()
    } else {
      addLog(`A/B launch failed: ${res.error || 'Unknown error'}`)
    }
  } catch (err) {
    addLog(`A/B launch error: ${err.message}`)
  } finally {
    abLaunching.value = false
  }
}

const startAbPolling = () => {
  abPollTimer = setInterval(pollAbStatus, 5000)
}
...
const pollAbStatus = async () => {
  if (!abCampaignId.value) return
  try {
    const res = await getAbStatus(abCampaignId.value)
    if (res.success && res.data) {
      abVariantStatuses.value = res.data.variants || abVariantStatuses.value
      const allDone = abVariantStatuses.value.every(v => v.status === 'completed' || v.status === 'failed')
      if (allDone) {
        abAllDone.value = true
        stopAbPolling()
        addLog('All variants completed. Recommendations ready.')
      }
    }
  } catch (err) {
    console.warn('AB poll error:', err)
  }
}

const goToReport = () => {
  if (abCampaignId.value) {
    router.push({ name: 'CampaignReport', params: { campaignId: abCampaignId.value } })
  }
}
