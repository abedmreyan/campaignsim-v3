  ...
  >
    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      <div
        ...
        :key="project._type === 'campaign' ? project.campaign_id : project.simulation_id"
        class="project-card"
        :class="{
          expanded: isExpanded,
          hovering: hoveringCard === index,
          'campaign-card': project._type === 'campaign'
        }"
      ...
      >
        ...
        <div class="card-header">
          <span class="card-id">
            <template v-if="project._type === 'campaign'">
              <span class="ab-badge">A/B</span>
              {{ truncateText(project.brand_name, 10) || 'Campaign' }}
            </template>
            <template v-else>{{ formatSimulationId(project.simulation_id) }}</template>
          </span>
          <div class="card-status-icons">
            <template v-if="project._type === 'campaign'">
              <span
                class="status-icon"
                :class="{ available: project.overall_status === 'completed', unavailable: project.overall_status !== 'completed' }"
                title="Variants completed"
              >◈</span>
              <span
                class="status-icon"
                :class="{ available: project.has_report, unavailable: !project.has_report }"
            ...
            <template v-else>
              ...
              >◇</span>
              <span class="status-icon available" :title="$t('history.envSetup')">◈</span>
              <span
                class="status-icon"
        ...
        <!-- Files area / variants area -->
        <div class="card-files-wrapper">
          <div class="corner-mark top-left-only"></div>

          <!-- Campaign: show variant chips -->
          <template v-if="project._type === 'campaign'">
            <div class="files-list" v-if="project.variants && project.variants.length">
              <div
                v-for="v in project.variants.slice(0, 3)"
                :key="v.variant_id"
                class="file-item"
              >
                <span class="file-tag" :class="getVariantStatusClass(v.status)">{{ v.channel?.toUpperCase() || 'VAR' }}</span>
                <span class="file-name">{{ truncateFilename(v.variant_name, 20) }}</span>
              </div>
              <div v-if="project.variants.length > 3" class="files-more">
                +{{ project.variants.length - 3 }} more variants
              </div>
            </div>
            <div class="files-empty" v-else>
              <span class="empty-file-icon">◈</span>
              <span class="empty-file-text">No variants</span>
            </div>
          </template>

          <!-- Simulation: show uploaded files -->
          <template v-else>
        ...
        <h3 class="card-title">
          <template v-if="project._type === 'campaign'">
            {{ truncateText(project.campaign_goal, 20) || 'A/B Campaign' }}
          </template>
          <template v-else>{{ getSimulationTitle(project.simulation_requirement) }}</template>
        </h3>

        <!-- Description -->
        <p class="card-desc">
          <template v-if="project._type === 'campaign'">
            {{ truncateText(project.campaign_goal, 55) }}
          </template>
          <template v-else>{{ truncateText(project.simulation_requirement, 55) }}</template>
        </p>
        ...
        <div class="card-footer">
          ...
          <span v-if="project._type === 'campaign'" class="card-progress" :class="getCampaignStatusClass(project.overall_status)">
            <span class="status-dot">●</span> {{ project.overall_status }}
          </span>
          ...
          <div class="modal-content">
            ...
            <div class="modal-header">
              <div class="modal-title-section">
                <template v-if="selectedProject._type === 'campaign'">
                  <span class="modal-id"><span class="ab-badge">A/B</span> {{ selectedProject.brand_name || 'Campaign' }}</span>
                  <span class="modal-progress" :class="getCampaignStatusClass(selectedProject.overall_status)">
                    <span class="status-dot">●</span> {{ selectedProject.overall_status }}
                  </span>
                </template>
                <template v-else>
            ...
            <template v-if="selectedProject._type === 'campaign'">
              <div class="modal-body">
                <div class="modal-section">
                  <div class="modal-label">Campaign Goal</div>
                  <div class="modal-requirement">{{ selectedProject.campaign_goal || '—' }}</div>
                </div>
                <div class="modal-section">
                  <div class="modal-label">Variants ({{ selectedProject.variant_count }})</div>
                  <div class="modal-files" v-if="selectedProject.variants && selectedProject.variants.length">
                    <div v-for="v in selectedProject.variants" :key="v.variant_id" class="modal-file-item">
                      <span class="file-tag" :class="getVariantStatusClass(v.status)">{{ v.channel?.toUpperCase() || 'VAR' }}</span>
                      <span class="modal-file-name">{{ v.variant_name }}</span>
                      <span class="variant-status-pill" :class="getVariantStatusClass(v.status)">{{ v.status }}</span>
                    </div>
                  </div>
                  <div class="modal-empty" v-else>No variants found</div>
                </div>
              </div>

              <div class="modal-divider">
                <span class="divider-line"></span>
                <span class="divider-text">Navigate</span>
                <span class="divider-line"></span>
              </div>

              <div class="modal-actions">
                <button class="modal-btn btn-campaign" @click="goToCampaign">
                  <span class="btn-step">A/B</span>
                  <span class="btn-icon">◈</span>
                  <span class="btn-text">View Campaign Dashboard</span>
                </button>
              </div>

              <div class="modal-playback-hint">
                <span class="hint-text">Opens the live campaign report and variant comparison.</span>
              </div>
            </template>

            <!-- ── Standard simulation modal body ── -->
            <template v-else>
              <div class="modal-body">
                <div class="modal-section">
...
import { getSimulationHistory, getCampaignHistory } from '../api/simulation'
...
const getCampaignStatusClass = (status) => {
  const map = { completed: 'completed', running: 'in-progress', failed: 'failed', pending: 'not-started' }
  return map[status] || 'not-started'
}

const getVariantStatusClass = (status) => {
  const map = { completed: 'xls', running: 'ppt', failed: 'pdf', pending: 'other' }
...
// Campaign dashboard
const goToCampaign = () => {
  if (selectedProject.value?.campaign_id) {
    router.push({
      name: 'CampaignReport',
      params: { campaignId: selectedProject.value.campaign_id }
    })
    closeModal()
  }
}
...
const loadHistory = async () => {
  try {
    loading.value = true
    const [simRes, campRes] = await Promise.allSettled([
      getSimulationHistory(20),
      getCampaignHistory(50),
    ])

    const sims = simRes.status === 'fulfilled' && simRes.value.success
      ? (simRes.value.data || []).map(s => ({ ...s, _type: 'simulation' }))
      : []

    const campaigns = campRes.status === 'fulfilled' && campRes.value.success
      ? (campRes.value.data || []).map(c => ({ ...c, _type: 'campaign' }))
      : []

    // Merge and sort newest-first
    const all = [...sims, ...campaigns].sort((a, b) => {
...
.campaign-card {
  border-color: #D1FAE5;
  background: linear-gradient(160deg, #FFFFFF 80%, #F0FDF4 100%);
}
...
.campaign-card .card-bottom-line {
...
.card-progress.failed { color: #EF4444; }
...
.variant-status-pill.xls  { background: #D1FAE5; color: #065F46; }
.variant-status-pill.ppt  { background: #FEF3C7; color: #92400E; }
.variant-status-pill.pdf  { background: #FEE2E2; color: #991B1B; }
.variant-status-pill.other { background: #F3F4F6; color: #6B7280; }

/* Campaign modal action button */
.modal-btn.btn-campaign {
  flex: 1;
  border-color: #D1FAE5;
  background: #F0FDF4;
}

.modal-btn.btn-campaign:hover:not(:disabled) {
  border-color: #10B981;
  background: #ECFDF5;
}

.modal-btn.btn-campaign .btn-icon { color: #10B981; }
