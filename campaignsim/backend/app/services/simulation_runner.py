...
logger = get_logger('campaignsim.simulation_runner')
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        if platform == "twitter":
            ...
            # Marketing channel simulation — all channels use the same script;
            # channel context is encoded in campaign_content inside the config.
            script_name = "run_channel_simulation.py"
            state.twitter_running = True  # uses Twitter substrate
        else:
        ...
        if not os.path.exists(script_path):
            raise ValueError(f" after : {script_path}")
    ...
    def stop_simulation(cls, simulation_id: str) -> SimulationRunState:
        ...
        if state.runner_status not in [RunnerStatus.RUNNING, RunnerStatus.PAUSED]:
            raise ValueError(f" after : {simulation_id}, status={state.runner_status}")
    ...
    def cleanup_simulation_logs(cls, simulation_id: str) -> Dict[str, Any]:
        ...
        logger.info(f": {simulation_id}, File: {cleaned_files}")
                ...
                if process.poll() is None:
                    ...
                    try:
                        ...
                        if os.path.exists(state_file):
                            ...
                            logger.warning(f"state.json after : {state_file}")
    ...
    def interview_all_agents(
        ...
        config_path = os.path.join(sim_dir, "simulation_config.json")
        if not os.path.exists(config_path):
            raise ValueError(f" after : {simulation_id}")
