<template>
  <div class="v3-root stagger-in">
    ...
    <DesignerChatPanel v-if="!store.isMockMode" class="v3-designer-slot" />
    ...
    <div class="v3-rail-wrap">
      ...
      <div class="v3-rail" ref="railRef" @scroll="onRailScroll">
        ...
        >
          ...
          <span class="v3-add-card__hint">{{ store.variants.length }}/6 created</span>
        </button>
        ...
        <div v-if="!store.variants.length" class="v3-empty">
          ...
          <p class="v3-empty__msg">Add 1–6 campaign variants to compare in simulation.</p>
        </div>
            ...
            <div class="v3-form">
              <div class="v3-form__row v3-form__row--2">
                ...
                <label class="v3-form__field">
                  ...
                  <select v-model="form.channel" class="v3-form__select" :disabled="loadingChannels" @change="onChannelChange">
                    <option value="">{{ loadingChannels ? "Loading channels…" : "Select channel" }}</option>
                    <option v-for="c in registryChannels" :key="c.key" :value="c.key">
                      {{ c.name }}{{ c.is_builtin ? "" : " (custom)" }}
                    </option>
                  </select>
                </label>
              </div>
              ...
              <div class="v3-form__row v3-form__row--2">
                <label class="v3-form__field">
                  ...
                  <select v-model="form.format" class="v3-form__select" :disabled="!selectedChannelDef">
                    <option value="">{{ selectedChannelDef ? "Select format" : "Select a channel first" }}</option>
              ...
              <label v-if="selectedChannelDef?.kind === 'direct'" class="v3-form__field">
...
const registryChannels = ref([]);
const loadingChannels = ref(false);

async function loadRegistryChannels() {
  loadingChannels.value = true;
  try {
    registryChannels.value = await getChannels();
  } catch {
    registryChannels.value = [];
  } finally {
    loadingChannels.value = false;
  }
}

const selectedChannelDef = computed(() => registryChannels.value.find((c) => c.key === form.channel) || null);
const availableFormats = computed(() => selectedChannelDef.value?.formats || []);
...
function validateForm() {
  ...
  if (selectedChannelDef.value?.kind === "direct" && !form.email_subject) errs.push("subject is required for this channel");
...
function submitForm() {
  ...
  if (editingVariant.value) {
    ...
    if (store.variants.length >= 6) {
      store.setNotice("Maximum 6 variants supported.");
      return;
    }
