...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const queryMatch = text.match(/(?:Analysis Query|Query):\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    
    const reqMatch = text.match(/(?:Campaign Goal|Prediction Scenario):\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    //  - ": X"
    const factMatch = text.match(/(?:Related Facts|Facts):\s*(\d+)/)
    const entityMatch = text.match(/(?:Entities):\s*(\d+)/)
    const relMatch = text.match(/(?:Relation Chains|Relations):\s*(\d+)/)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])
    
    //  - 
    const subQSection = text.match(/### (?:Sub-Questions|Analysis Sub-Questions)\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
    ...
    //  - 
    const factsSection = text.match(/### (?:Key Facts|Critical Facts)[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (factsSection) {
    ...
    //  - 
    const entitySection = text.match(/### (?:Core Entities|Key Entities)\n([\s\S]*?)(?=\n###|$)/)
    if (entitySection) {
    ...
    //  - 
    const relSection = text.match(/### (?:Relation Chains|Relations)\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    const histMatch = text.match(/(?:Historical Facts):\s*(\d+)/)
    ...
    // / - 
    const histSection = text.match(/### (?:Historical Facts)[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (histSection) {
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const topicMatch = text.match(/\*\*(?:Interview Topic|Topic):\*\*\s*(.+?)(?:\n|$)/)
    if (topicMatch) result.topic = topicMatch[1].trim()
    
    //  "5 / 9 Agent"
    const countMatch = text.match(/\*\*(?:Interviews Conducted|Agent Count):\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    const reasonMatch = text.match(/### (?:Agent Selection Reasons|Selection Reasoning)\n([\s\S]*?)(?=\n---\n|\n### (?:Interview Transcripts|Interview Transcripts))/)
    if (reasonMatch) {
    ...
    const interviewBlocks = text.split(/#### (?:Interview|Agent) #\d+:/).slice(1)
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      const bioMatch = block.match(/_(?:Bio|Background)\s*([\s\S]*?)_\n/)
      if (bioMatch) {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          //  Reddit 
          if (interview.redditAnswer && interview.redditAnswer && !['(No response)'].includes(interview.redditAnswer)) {
            interview.twitterAnswer = interview.redditAnswer
          }
        } else if (twitterMatch && !redditMatch) {
          if (interview.twitterAnswer && interview.twitterAnswer && !['(No response)'].includes(interview.twitterAnswer)) {
    ...
    const summaryMatch = text.match(/### (?:Interview Summary|Key Observations)\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const queryMatch = text.match(/(?:Search Query|Query):\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    ...
    const edgesSection = text.match(/### (?:Related Edges|Relations)\n([\s\S]*?)(?=\n###|$)/)
    if (edgesSection) {
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(No response from platform)' || t === '[No reply]' || t === '(No response)'
    }
...
:deep(.interview-display .agent-tab.active) {
  background: linear-gradient(135deg, #E0F7F5 0%, #CCF0ED 100%);
  border-color: #7DE5DF;
  color: #0A8F82;
  box-shadow: 0 1px 2px rgba(10, 191, 173, 0.1);
}
